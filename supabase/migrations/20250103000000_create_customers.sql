...
CREATE POLICY "Sellers can view their own customers" ON public.customers
    FOR SELECT USING (seller_id = auth.uid());

CREATE POLICY "Sellers can insert their own customers" ON public.customers
    FOR INSERT WITH CHECK (seller_id = auth.uid());

CREATE POLICY "Sellers can update their own customers" ON public.customers
    FOR UPDATE USING (seller_id = auth.uid());

CREATE POLICY "Sellers can delete their own customers" ON public.customers
    FOR DELETE USING (seller_id = auth.uid());

-- Function to update customer stats when orders are created/updated
-- Note: This function will be used by triggers after orders table is created
CREATE OR REPLACE FUNCTION update_customer_stats()
RETURNS TRIGGER AS $$
BEGIN
    IF TG_OP = 'INSERT' OR TG_OP = 'UPDATE' THEN
        -- Update customer stats for new/updated orders
        UPDATE customers 
        SET 
            total_orders = (
                SELECT COUNT(*) 
...
-- Function to add a retailer as a customer
