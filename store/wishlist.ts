...
export const useWishlistStore = create<WishlistStore>((set, get) => ({
  ...
  loadWishlist: async () => {
    ...
    try {
      ...
      // Fetch seller details for all products
      ...
      if (sellerIds.length > 0) {
        try {
          ...
          if (sellers) {
            sellers.forEach(s => {
              sellerMap[s.user_id] = s.business_name || 'Unknown Seller';
            });
          }
        } catch (err) {
      ...
      const wishlistItems = productsData.map(product => ({
        ...
        seller_name: sellerMap[product.seller_id] || 'Unknown Seller'
      }));

      console.log('Transformed wishlist items:', wishlistItems);
      set({ items: wishlistItems, error: null });
    } catch (error) {
      console.error('Error in loadWishlist:', error);
      set({ items: [], loading: false, error: error instanceof Error ? error.message : 'Failed to load wishlist' });
    } finally {
      set({ loading: false });
    }
  },
