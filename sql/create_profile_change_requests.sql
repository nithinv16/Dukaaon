...
-- Changes to sensitive fields (phone, GST, address, business name, owner name, email, location) require approval
...
CREATE TABLE public.profile_change_requests (
    ...
    -- email, address, gst_number, latitude, longitude, shop_name for retailers)
...
BEGIN
    ...
    IF req.user_role = 'seller' THEN
        ...
        -- Build profile changes for seller
        IF changes ? 'phone_number' THEN
            profile_changes := profile_changes || jsonb_build_object('phone_number', changes->>'phone_number');
        END IF;
        IF changes ? 'email' THEN
            profile_changes := profile_changes || jsonb_build_object('email', changes->>'email');
        END IF;
    ...
    ELSIF req.user_role = 'retailer' THEN
        ...
        -- Build profile changes for retailer
        IF changes ? 'phone_number' THEN
            profile_changes := profile_changes || jsonb_build_object('phone_number', changes->>'phone_number');
        END IF;
        IF changes ? 'email' THEN
    ...
    -- Apply profile changes (phone, email, location) to profiles table
    IF profile_changes != '{}'::jsonb THEN
        UPDATE public.profiles
        SET 
            phone_number = COALESCE((profile_changes->>'phone_number'), phone_number),
            email = COALESCE((profile_changes->>'email'), email),
...
-- View pending requests (for admin dashboard)
-- SELECT 
--     pcr.id,
--     pcr.user_id,
--     pcr.user_role,
